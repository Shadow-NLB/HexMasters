...
class IFrame :
	...
	"""
	def __init__(self, parentFrame, parentPosition, dimensions, color) :
		self.pFrame = parentFrame
		self.pPos = parentPosition
		self.xSize, self.ySize = dimensions
		self.color = color
	...
	def getRelativePos(self, worldCoords) :
		...
		if (self.pFrame != None) :
			...
			if (pointInBox(relPos, (self.xSize, self.ySize))) :
				return relPos
			else :
	...
	def renderFrame(self, surface) :
		pygame.draw.rect(surface, self.color, (0, 0, self.xSize, self.ySize))
